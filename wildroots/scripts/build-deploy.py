...
# agent-addressable moat artifacts (generated by scripts/extract-corpus.mjs)
for artifact in ("corpus.json", "llms.txt"):
    a = ROOT / "dig-deeper" / artifact
    if a.exists():
        shutil.copy2(a, DEPLOY / artifact)
    else:
        print(f"  note: {artifact} not found — run `node scripts/extract-corpus.mjs` first", file=sys.stderr)
